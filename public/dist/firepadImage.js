/*!
 * firepadImage is an open-source, collaborative code and text editor. It was designed
...
})('firepadImage', function () {var firepadImage = firepadImage || { };
firepadImage.utils = { };

firepadImage.utils.makeEventEmitter = function(clazz, opt_allowedEVents) {
...
firepadImage.utils.elt = function(tag, content, attrs) {
  var e = document.createElement(tag);
  if (typeof content === "string") {
    firepadImage.utils.setTextContent(e, content);
  } else if (content) {
...
firepadImage.utils.setTextContent = function(e, str) {
  e.innerHTML = "";
  e.appendChild(document.createTextNode(str));
};


firepadImage.utils.on = function(emitter, type, f, capture) {
...
firepadImage.utils.off = function(emitter, type, f, capture) {
...
firepadImage.utils.preventDefault = function(e) {
  if (e.preventDefault) {
    e.preventDefault();
  } else {
    e.returnValue = false;
...
firepadImage.utils.stopPropagation = function(e) {
...
firepadImage.utils.stopEvent = function(e) {
  firepadImage.utils.preventDefault(e);
  firepadImage.utils.stopPropagation(e);
};

firepadImage.utils.stopEventAnd = function(fn) {
  return function(e) {
    fn(e);
    firepadImage.utils.stopEvent(e);
    return false;
  };
};

firepadImage.utils.trim = function(str) {
  return str.replace(/^\s+/g, '').replace(/\s+$/g, '');
};

firepadImage.utils.stringEndsWith = function(str, suffix) {
...
firepadImage.utils.assert = function assert (b, msg) {
  if (!b) {
    throw new Error(msg || "assertion error");
  }
};

firepadImage.utils.log = function() {
  if (typeof console !== 'undefined' && typeof console.log !== 'undefined') {
    var args = ['firepadImage:'];
...
var firepadImage = firepadImage || { };
firepadImage.Span = (function () {
...
firepadImage.TextOp = (function() {
  var utils = firepadImage.utils;
...
firepadImage.TextOperation = (function () {
  'use strict';
  var TextOp = firepadImage.TextOp;
  var utils = firepadImage.utils;
...
firepadImage.AnnotationList = (function () {
  var Span = firepadImage.Span;
...
firepadImage.FirebaseAdapter = (function (global) {

  if (typeof firebase === "undefined" && typeof require === 'function' && typeof Firebase !== 'function') {
    firebase = require('firebase');
  }

  var TextOperation = firepadImage.TextOperation;
  var utils = firepadImage.utils;
...
firepadImage.RichTextToolbar = (function(global) {
  var utils = firepadImage.utils;
  ...
  RichTextToolbar.prototype.makeButton_ = function(eventName, iconName) {
    ...
    var btn = utils.elt('a', [utils.elt('span', '', { 'class': 'firepadImage-tb-' + iconName } )], { 'class': 'firepadImage-btn' });
  ...
  RichTextToolbar.prototype.makeElement_ = function() {
    ...
    var toolbarOptions = [
      // utils.elt('div', [font], { 'class': 'firepadImage-btn-group'}),
      // utils.elt('div', [fontSize], { 'class': 'firepadImage-btn-group'}),
      // utils.elt('div', [color], { 'class': 'firepadImage-btn-group'}),
      // utils.elt('div', [self.makeButton_('bold'), self.makeButton_('italic'), self.makeButton_('underline'), self.makeButton_('strike', 'strikethrough')], { 'class': 'firepadImage-btn-group'}),
      // utils.elt('div', [self.makeButton_('unordered-list', 'list-2'), self.makeButton_('ordered-list', 'numbered-list'), self.makeButton_('todo-list', 'list')], { 'class': 'firepadImage-btn-group'}),
      // utils.elt('div', [self.makeButton_('indent-decrease'), self.makeButton_('indent-increase')], { 'class': 'firepadImage-btn-group'}),
      // utils.elt('div', [self.makeButton_('left', 'paragraph-left'), self.makeButton_('center', 'paragraph-center'), self.makeButton_('right', 'paragraph-right')], { 'class': 'firepadImage-btn-group'}),
      // utils.elt('div', [self.makeButton_('undo'), self.makeButton_('redo')], { 'class': 'firepadImage-btn-group'})
    ];

    // if (self.imageInsertionUI) {
    //   toolbarOptions.push(utils.elt('div', [self.makeButton_('insert-image')], { 'class': 'firepadImage-btn-group' }));
    // }
  ...
  RichTextToolbar.prototype.makeFontDropdown_ = function() {
    // NOTE: There must be matching .css styles in firepadImage.css.
  ...
  //     content.className = 'firepadImage-color-dropdown-item';
  ...
  RichTextToolbar.prototype.makeDropdown_ = function(title, eventName, items, value_suffix) {
    ...
    var self = this;
    var button = utils.elt('a', title + ' \u25be', { 'class': 'firepadImage-btn firepadImage-dropdown' });
    var list = utils.elt('ul', [ ], { 'class': 'firepadImage-dropdown-menu' });
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////HACKED : changer de typo onclick sur le document and 
...
var firepadImage = firepadImage || { };
firepadImage.WrappedOperation = (function (global) {
  'use strict';
...
firepadImage.UndoManager = (function () {
  'use strict';
...
firepadImage.EditorClient = (function () {
  'use strict';

  var Client = firepadImage.Client;
  var Cursor = firepadImage.Cursor;
  var UndoManager = firepadImage.UndoManager;
  var WrappedOperation = firepadImage.WrappedOperation;
...
firepadImage.utils.makeEventEmitter(firepadImage.EditorClient, ['synced']);

var firepadImage,
  __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
  __slice = [].slice;

if (typeof firepadImage === "undefined" || firepadImage === null) {
  firepadImage = {};
}

firepadImage.ACEAdapter = (function() {
  ...
  ACEAdapter.prototype.operationFromACEChange = function(change) {
    ...
    insert_op = new firepadImage.TextOperation().retain(start).insert(text).retain(restLength);
    delete_op = new firepadImage.TextOperation().retain(start)["delete"](text).retain(restLength);
    if (change.action === 'remove') {
  ...
  ACEAdapter.prototype.getCursor = function() {
    ...
    return new firepadImage.Cursor(start, end);
  };
...
firepadImage.AttributeConstants = {
  BOLD: 'b',
  ITALIC: 'i',
  UNDERLINE: 'u',
  STRIKE: 's',
...
firepadImage.sentinelConstants = {
...
firepadImage.EntityManager = (function () {
  var utils = firepadImage.utils;
  ...
  EntityManager.prototype.register = function(type, options) {
    firepadImage.utils.assert(options.render, "Entity options should include a 'render' function!");
    firepadImage.utils.assert(options.fromElement, "Entity options should include a 'fromElement' function!");
  ...
  EntityManager.prototype.exportToElement = function(entity) {
    ...
    elt.setAttribute('data-firepadImage-entity', entity.type);
    return elt;
  };
  ...
  */
  ...
  EntityManager.prototype.fromElement = function(element) {
    var type = element.getAttribute('data-firepadImage-entity');
    ...
    if (type && this.entities_[type]) {
      ...
      return new firepadImage.Entity(type, info);
    }
  };
  ...
  EntityManager.prototype.tryRenderToElement_ = function(entity, renderFn, entityHandle) {
    ...
    if (this.entities_[type] && this.entities_[type][renderFn]) {
      var windowDocument = firepadImage.document || (window && window.document);
      var res = this.entities_[type][renderFn](info, entityHandle, windowDocument);
      if (res) {
        if (typeof res === 'string') {
          var div = (firepadImage.document || document).createElement('div');
          div.innerHTML = res;
          return div.childNodes[0];
        } else if (typeof res === 'object') {
          firepadImage.utils.assert(typeof res.nodeType !== 'undefined', 'Error rendering ' + type + ' entity.  render() function' +
 ...
 */
firepadImage.Entity = (function() {
  var ATTR = firepadImage.AttributeConstants;
...
firepadImage.RichTextCodeMirror = (function () {
  var AnnotationList = firepadImage.AnnotationList;
  var Span = firepadImage.Span;
  var utils = firepadImage.utils;
  var ATTR = firepadImage.AttributeConstants;
  ...
  var LineSentinelCharacter   = firepadImage.sentinelConstants.LINE_SENTINEL_CHARACTER;
  var EntitySentinelCharacter = firepadImage.sentinelConstants.ENTITY_SENTINEL_CHARACTER;
  ...
  RichTextCodeMirror.prototype.insertEntityAt = function(index, type, info, origin) {
    var cm = this.codeMirror;
    this.insertEntity_(index, new firepadImage.Entity(type, info), origin);
  };
  ...
  RichTextCodeMirror.prototype.getClassNameForAttributes_ = function(attributes) {
    ...
    for (var attr in attributes) {
      ...
      if (attr === ATTR.LINE_SENTINEL) {
        firepadImage.utils.assert(val === true, "LINE_SENTINEL attribute should be true if it exists.");
      } else {
  ...
  RichTextCodeMirror.prototype.markEntity_ = function(annotationNode) {
    ...
    var entity = firepadImage.Entity.fromAttributes(attributes);
    var cm = this.codeMirror;
    var self = this;
    ...
    annotationNode.attachObject({
      ...
      update: function(info) {
        var entity = firepadImage.Entity.fromAttributes(info);
        for(var i = 0; i < markers.length; i++) {
  ...
  RichTextCodeMirror.prototype.markLineSentinelCharactersForChanges_ = function(changes) {
    ...
    // But In practice, people using firepadImage for rich-text editing don't batch multiple changes
  ...
  RichTextCodeMirror.prototype.updateCurrentAttributes_ = function() {
    ...
    if (spans.length > 0 && (!(ATTR.LINE_SENTINEL in spans[0].annotation.attributes))) {
      ...
      firepadImage.utils.assert(!(ATTR.LINE_SENTINEL in spans[1].annotation.attributes), "Cursor can't be between two line sentinel characters.");
  ...
  RichTextCodeMirror.prototype.getLineAttributes_ = function(lineNum) {
    ...
    if (line.length > 0 && line[0] === LineSentinelCharacter) {
      ...
      firepadImage.utils.assert(spans.length === 1);
  ...
  RichTextCodeMirror.prototype.deleteRight = function() {
    ...
    //   // (see https://github.com/firebase/firepadImage/issues/209).
...
firepadImage.RichTextCodeMirrorAdapter = (function () {
  'use strict';
 ...
 * @type {Function}
 */
firepadImage.Formatting = (function() {
  var ATTR = firepadImage.AttributeConstants;
 ...
 * @type {Function}
 */
firepadImage.LineFormatting = (function() {
  var ATTR = firepadImage.AttributeConstants;
  ...
  LineFormatting.prototype.listItem = function(val) {
    firepadImage.utils.assert(val === false || val === 'u' || val === 'o' || val === 't' || val === 'tc');
...
firepadImage.Line = (function() {
  function Line(textPieces, formatting) {
    ...
    this.formatting = formatting || firepadImage.LineFormatting();
  }
...
/**
 * Helper to parse html into firepadImage-compatible lines / text.
 * @type {*}
 */
firepadImage.ParseHtml = (function () {
  var LIST_TYPE = firepadImage.LineFormatting.LIST_TYPE;
  ...
  function ParseState(opt_listType, opt_lineFormatting, opt_textFormatting) {
    ...
    this.lineFormatting = opt_lineFormatting || firepadImage.LineFormatting();
    this.textFormatting = opt_textFormatting || firepadImage.Formatting();
  }
  ...
  ParseOutput.prototype.newline = function(state) {
    ...
    this.lines.push(firepadImage.Line(this.currentLine, lineFormatting));
    this.currentLine = [];
  };
  ...
  function parseNode(node, state, output) {
    ...
    if (node.nodeType === Node.ELEMENT_NODE) {
      ...
      if (entity) {
        output.currentLine.push(new firepadImage.Text(
            firepadImage.sentinelConstants.ENTITY_SENTINEL_CHARACTER,
            new firepadImage.Formatting(entity.toAttributes())
        ));
        return;
      }
    }
    ...
    switch (node.nodeType) {
      case Node.TEXT_NODE:
        ...
        output.currentLine.push(firepadImage.Text(text, state.textFormatting));
        break;
      case Node.ELEMENT_NODE:
        ...
        switch (node.nodeName.toLowerCase()) {
          ...
          case 'ul':
            ...
            var listType = node.getAttribute('class') === 'firepadImage-todo' ? LIST_TYPE.TODO : LIST_TYPE.UNORDERED;
  ...
  function parseListItem(node, state, output) {
    ...
    var listType = (node.getAttribute('class') === 'firepadImage-checked') ? LIST_TYPE.TODOCHECKED : state.listType;
  ...
  function parseStyle(state, styleString) {
    ...
    for(var i = 0; i < styles.length; i++) {
      ...
      if (stylePieces.length !== 2)
        continue;
      var prop = firepadImage.utils.trim(stylePieces[0]).toLowerCase();
      var val = firepadImage.utils.trim(stylePieces[1]).toLowerCase();
      switch (prop) {
        ...
        case 'font-size':
          ...
          if (firepadImage.utils.stringEndsWith(val, allowedValues)) {
            size = val;
          }
          ...
          break;
        case 'font-family':
          var font = firepadImage.utils.trim(val.split(',')[0]); // get first font.
 ...
 */
firepadImage.SerializeHtml = (function () {

  var utils      = firepadImage.utils;
  var ATTR       = firepadImage.AttributeConstants;
  var LIST_TYPE  = firepadImage.LineFormatting.LIST_TYPE;
  var TODO_STYLE = '<style>ul.firepadImage-todo { list-style: none; margin-left: 0; padding-left: 0; } ul.firepadImage-todo > li { padding-left: 1em; text-indent: -1em; } ul.firepadImage-todo > li:before { content: "\\2610"; padding-right: 5px; } ul.firepadImage-todo > li.firepadImage-checked:before { content: "\\2611"; padding-right: 5px; }</style>\n';

  function open(listType) {
    return (listType === LIST_TYPE.ORDERED) ? '<ol>' :
           (listType === LIST_TYPE.UNORDERED) ? '<ul>' :
           '<ul class="firepadImage-todo">';
  }
    ...
    while(op) {
        ...
        if (listType) {
          ...
          switch (listType)
          {
            case LIST_TYPE.TODOCHECKED:
              clazz = ' class="firepadImage-checked"';
              break;
            case LIST_TYPE.TODO:
              clazz = ' class="firepadImage-unchecked"';
              break;
          }
          html += "<li" + clazz + style + ">";
          inListItem = true;
      ...
      if (ATTR.ENTITY_SENTINEL in attrs) {
        for(var j = 0; j < op.text.length; j++) {
          var entity = firepadImage.Entity.fromAttributes(attrs);
      ...
      if (newLineIndex >= 0) {
        ...
        if (newLineIndex < text.length - 1) {
          // split op.
          op = new firepadImage.TextOp('insert', text.substr(newLineIndex+1), attrs);
        } else {
          op = doc.ops[++i];
        }
 ...
 */
firepadImage.textPiecesToInserts = function(atNewLine, textPieces) {
  var inserts = [];

  function insert(string, attributes) {
    if (string instanceof firepadImage.Text) {
  ...
  function insertLine(line, withNewline) {
    ...
    if (atNewLine) {
      insert(firepadImage.sentinelConstants.LINE_SENTINEL_CHARACTER, line.formatting.attributes);
    }
  ...
  for(var i = 0; i < textPieces.length; i++) {
    if (textPieces[i] instanceof firepadImage.Line) {
...
/**
 * Instance of headless firepadImage for use in NodeJS. Supports get/set on text/html.
 */
firepadImage.Headless = (function() {
  var TextOperation   = firepadImage.TextOperation;
  var FirebaseAdapter = firepadImage.FirebaseAdapter;
  var EntityManager   = firepadImage.EntityManager;
  var ParseHtml       = firepadImage.ParseHtml;
  ...
  Headless.prototype.getText = function(callback) {
    if (this.zombie_) {
      throw new Error('You can\'t use a firepadImage.Headless after calling dispose()!');
    }
    ...
    this.getDocument(function(doc) {
      ...
      for (key in firepadImage.sentinelConstants) {
        text = text.replace(new RegExp(firepadImage.sentinelConstants[key], 'g'), '');
      }
      callback(text);
    });
  }
  ...
  Headless.prototype.initializeFakeDom = function(callback) {
    if (typeof document === 'object' || typeof firepadImage.document === 'object') {
      callback();
    } else {
      require('jsdom').env('<head></head><body></body>', function(err, window) {
        if (firepadImage.document) {
        ...
        firepadImage.document = window.document;
        callback();
      });
    }
  }
  ...
  Headless.prototype.getHtml = function(callback) {
    ...
    self.initializeFakeDom(function() {
      self.getDocument(function(doc) {
        callback(firepadImage.SerializeHtml(doc, self.entityManager_));
      });
    });
  }
  ...
  Headless.prototype.setHtml = function(html, callback) {
    ...
    self.initializeFakeDom(function() {
      ...
      var inserts    = firepadImage.textPiecesToInserts(true, textPieces);
...
firepadImage.firepadImage = (function(global) {
  if (!firepadImage.RichTextCodeMirrorAdapter) {
    throw new Error("Oops! It looks like you're trying to include lib/firepadImage.js directly.  This is actually one of many source files that make up firepadImage.  You want dist/firepadImage.js instead.");
  }
  var RichTextCodeMirrorAdapter = firepadImage.RichTextCodeMirrorAdapter;
  var RichTextCodeMirror = firepadImage.RichTextCodeMirror;
  var RichTextToolbar = firepadImage.RichTextToolbar;
  var ACEAdapter = firepadImage.ACEAdapter;
  var FirebaseAdapter = firepadImage.FirebaseAdapter;
  var EditorClient = firepadImage.EditorClient;
  var EntityManager = firepadImage.EntityManager;
  var ATTR = firepadImage.AttributeConstants;
  var utils = firepadImage.utils;
  var LIST_TYPE = firepadImage.LineFormatting.LIST_TYPE;
  ...
  function firepadImage(ref, place, options) {
    if (!(this instanceof firepadImage)) { return new firepadImage(ref, place, options); }
    ...
    if (CodeMirror && place instanceof CodeMirror) {
      ...
      if (curValue !== '') {
        throw new Error("Can't initialize firepadImage with a CodeMirror instance that already contains text.");
      }
    } else if (ace && place && place.session instanceof ace.EditSession) {
      this.ace_ = this.editor_ = place;
      curValue = this.ace_.getValue();
      if (curValue !== '') {
        throw new Error("Can't initialize firepadImage with an ACE instance that already contains text.");
      }
    } else {
    ...
    this.firepadImageWrapper_ = utils.elt("div", null, { 'class': 'firepadImage' });
    editorWrapper.parentNode.replaceChild(this.firepadImageWrapper_, editorWrapper);
    this.firepadImageWrapper_.appendChild(editorWrapper);

    // Don't allow drag/drop because it causes issues.  See https://github.com/firebase/firepadImage/issues/36
    utils.on(editorWrapper, 'dragstart', utils.stopEvent);

    // Provide an easy way to get the firepadImage instance associated with this CodeMirror instance.
    this.editor_.firepadImage = this;
    ...
    if (this.getOption('richTextShortcuts', false)) {
      ...
      this.firepadImageWrapper_.className += ' firepadImage-richtext';
    }
    ...
    if (this.getOption('richTextToolbar', false)) {
      this.addToolbar_();
      this.firepadImageWrapper_.className += ' firepadImage-richtext firepadImage-with-toolbar';
    }
    ...
    if (this.codeMirror_) {
      this.richTextCodeMirror_ = new RichTextCodeMirror(this.codeMirror_, this.entityManager_, { cssPrefix: 'firepadImage-' });
  ...
  utils.makeEventEmitter(firepadImage);

  // For readability, these are the primary "constructors", even though right now they're just aliases for firepadImage.
  firepadImage.fromCodeMirror = firepadImage;
  firepadImage.fromACE = firepadImage;

  firepadImage.prototype.dispose = function() {
    this.zombie_ = true; // We've been disposed.  No longer valid to do anything.

    // Unwrap the editor.
    var editorWrapper = this.codeMirror_ ? this.codeMirror_.getWrapperElement() : this.ace_.container;
    this.firepadImageWrapper_.removeChild(editorWrapper);
    this.firepadImageWrapper_.parentNode.replaceChild(editorWrapper, this.firepadImageWrapper_);

    this.editor_.firepadImage = null;
  ...
  firepadImage.prototype.setUserId = function(userId) {
    this.firebaseAdapter_.setUserId(userId);
  };

  firepadImage.prototype.setUserColor = function(color) {
    this.firebaseAdapter_.setColor(color);
  };

  firepadImage.prototype.getText = function() {
  ...
  firepadImage.prototype.setText = function(textPieces) {
  ...
  firepadImage.prototype.insertTextAtCursor = function(textPieces) {
    this.insertText(this.codeMirror_.indexFromPos(this.codeMirror_.getCursor()), textPieces);
  };

  firepadImage.prototype.insertText = function(index, textPieces) {
    ...
    self.codeMirror_.operation(function() {
      ...
      var inserts = firepadImage.textPiecesToInserts(atNewLine, textPieces);
  ...
  firepadImage.prototype.getHtml = function() {
    return this.getHtmlFromRange(null, null);
  };

  firepadImage.prototype.getHtmlFromSelection = function() {
  ...
  firepadImage.prototype.getHtmlFromRange = function(start, end) {
    ...
    return firepadImage.SerializeHtml(doc, this.entityManager_);
  };

  firepadImage.prototype.insertHtml = function (index, html) {
    var lines = firepadImage.ParseHtml(html, this.entityManager_);
    this.insertText(index, lines);
  };

  firepadImage.prototype.insertHtmlAtCursor = function (html) {
    this.insertHtml(this.codeMirror_.indexFromPos(this.codeMirror_.getCursor()), html);
  };

  firepadImage.prototype.setHtml = function (html) {
    var lines = firepadImage.ParseHtml(html, this.entityManager_);
    this.setText(lines);
  };

  firepadImage.prototype.isHistoryEmpty = function() {
    this.assertReady_('isHistoryEmpty');
    return this.firebaseAdapter_.isHistoryEmpty();
  };

  firepadImage.prototype.bold = function() {
    this.richTextCodeMirror_.toggleAttribute(ATTR.BOLD);
    this.codeMirror_.focus();
  };

  firepadImage.prototype.italic = function() {
    this.richTextCodeMirror_.toggleAttribute(ATTR.ITALIC);
    this.codeMirror_.focus();
  };

  firepadImage.prototype.underline = function() {
    this.richTextCodeMirror_.toggleAttribute(ATTR.UNDERLINE);
    this.codeMirror_.focus();
  };

  firepadImage.prototype.strike = function() {
    this.richTextCodeMirror_.toggleAttribute(ATTR.STRIKE);
    this.codeMirror_.focus();
  };

  firepadImage.prototype.fontSize = function(size) {
    this.richTextCodeMirror_.setAttribute(ATTR.FONT_SIZE, size);
    this.codeMirror_.focus();
  };

  firepadImage.prototype.font = function(font) {
    this.richTextCodeMirror_.setAttribute(ATTR.FONT, font);
    this.codeMirror_.focus();

  };

  firepadImage.prototype.color = function(color) {
    this.richTextCodeMirror_.setAttribute(ATTR.COLOR, color);
    this.codeMirror_.focus();
  };

  firepadImage.prototype.highlight = function() {
    // this.richTextCodeMirror_.toggleAttribute(ATTR.BACKGROUND_COLOR, 'rgba(255,255,0,.65)');
    // this.codeMirror_.focus();
    console.log("No highlight sorry");
  };

  firepadImage.prototype.align = function(alignment) {
  ...
  firepadImage.prototype.orderedList = function() {
    this.richTextCodeMirror_.toggleLineAttribute(ATTR.LIST_TYPE, 'o');
    this.codeMirror_.focus();
  };

  firepadImage.prototype.unorderedList = function() {
    this.richTextCodeMirror_.toggleLineAttribute(ATTR.LIST_TYPE, 'u');
    this.codeMirror_.focus();
  };

  firepadImage.prototype.todo = function() {
    this.richTextCodeMirror_.toggleTodo();
    this.codeMirror_.focus();
  };

  firepadImage.prototype.newline = function() {
  ...
  firepadImage.prototype.deleteLeft = function() {
    // this.richTextCodeMirror_.deleteLeft();
    console.log("Can't delete sorry!")
  };

  firepadImage.prototype.deleteRight = function() {
    console.log("Can't delete !")
    // this.richTextCodeMirror_.deleteRight();
  };

  firepadImage.prototype.indent = function() {
    this.richTextCodeMirror_.indent();
    this.codeMirror_.focus();
  };

  firepadImage.prototype.unindent = function() {
    this.richTextCodeMirror_.unindent();
    this.codeMirror_.focus();
  };

  firepadImage.prototype.undo = function() {
    this.codeMirror_.undo();
  };

  firepadImage.prototype.redo = function() {
    this.codeMirror_.redo();
  };

  firepadImage.prototype.insertEntity = function(type, info, origin) {
    this.richTextCodeMirror_.insertEntityAtCursor(type, info, origin);
  };

  firepadImage.prototype.insertEntityAt = function(index, type, info, origin) {
    this.richTextCodeMirror_.insertEntityAt(index, type, info, origin);
  };

  firepadImage.prototype.registerEntity = function(type, options) {
    this.entityManager_.register(type, options);
  };

  firepadImage.prototype.getOption = function(option, def) {
    return (option in this.options_) ? this.options_[option] : def;
  };

  firepadImage.prototype.assertReady_ = function(funcName) {
    if (!this.ready_) {
      throw new Error('You must wait for the "ready" event before calling ' + funcName + '.');
    }
    if (this.zombie_) {
      throw new Error('You can\'t use a firepadImage after calling dispose()!  [called ' + funcName + ']');
    }
  };

  firepadImage.prototype.makeImageDialog_ = function() {
    this.makeDialog_('img', 'Insert image url');
  };

  firepadImage.prototype.makeDialog_ = function(id, placeholder) {
   var self = this;

   var hideDialog = function() {
     var dialog = document.getElementById('overlay');
     dialog.style.visibility = "hidden";
     self.firepadImageWrapper_.removeChild(dialog);
   };
   ...
   var input = utils.elt('input', null, { 'class':'firepadImage-dialog-input', 'id':id, 'type':'text', 'placeholder':placeholder, 'autofocus':'autofocus' });

   var submit = utils.elt('a', 'Submit', { 'class': 'firepadImage-btn', 'id':'submitbtn' });
   utils.on(submit, 'click', utils.stopEventAnd(cb));

   var cancel = utils.elt('a', 'Cancel', { 'class': 'firepadImage-btn' });
   utils.on(cancel, 'click', utils.stopEventAnd(hideDialog));

   var buttonsdiv = utils.elt('div', [submit, cancel], { 'class':'firepadImage-btn-group' });

   var div = utils.elt('div', [input, buttonsdiv], { 'class':'firepadImage-dialog-div' });
   var dialog = utils.elt('div', [div], { 'class': 'firepadImage-dialog', id:'overlay' });

   this.firepadImageWrapper_.appendChild(dialog);
  };

  firepadImage.prototype.addToolbar_ = function() {
    ...
    this.firepadImageWrapper_.insertBefore(this.toolbar.element(), this.firepadImageWrapper_.firstChild);
  };



  // firepadImage.prototype.addPoweredByLogo_ = function() {
  //   var poweredBy = utils.elt('a', null, { 'class': 'powered-by-firepadImage'} );
  //   poweredBy.setAttribute('href', 'http://www.firepadImage.io/');
  //   poweredBy.setAttribute('target', '_blank');
  //   this.firepadImageWrapper_.appendChild(poweredBy)
  // };




  firepadImage.prototype.initializeKeyMap_ = function() {
    function binder(fn) {
      return function(cm) {
...
// Export Text classes
firepadImage.firepadImage.Formatting = firepadImage.Formatting;
firepadImage.firepadImage.Text = firepadImage.Text;
firepadImage.firepadImage.Entity = firepadImage.Entity;
firepadImage.firepadImage.LineFormatting = firepadImage.LineFormatting;
firepadImage.firepadImage.Line = firepadImage.Line;
firepadImage.firepadImage.TextOperation = firepadImage.TextOperation;
firepadImage.firepadImage.Headless = firepadImage.Headless;

// Export adapters
firepadImage.firepadImage.RichTextCodeMirrorAdapter = firepadImage.RichTextCodeMirrorAdapter;
firepadImage.firepadImage.ACEAdapter = firepadImage.ACEAdapter;

return firepadImage.firepadImage; }, this);
